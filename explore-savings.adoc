...
+
Workload Factory consolidates multiple EBS hosts into a single FSx for ONTAP configuration recommendation to optimize cost savings unless the selected EBS hosts exceed throughput, capacity, or IOPS limits for a single FSx for ONTAP file system.
--
.Amazon FSx for Windows File Server
--
.Steps
