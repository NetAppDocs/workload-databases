...
.. *Add hosts*: Optionally, select additional detected EBS hosts to include in the savings calculation.
+
Depending on the size of the hosts, Workload Factory might consolidate multiple EBS hosts into a single FSx for ONTAP configuration recommendation to optimize cost savings.
--
.Amazon FSx for Windows File Server
--
.Steps
