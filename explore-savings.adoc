...
If you have enabled _automate_ mode (link:https://docs.netapp.com/us-en/workload-setup-admin/operational-modes.html[learn more about modes]) by adding AWS credentials to your Workload Factory account, then Workload Factory can identify all the storage systems running Microsoft SQL Server that are available in your AWS account. The AWS account that is searched is based on the credentials you've defined in your Workload Factory account. You can explore savings automatically.
...
You can also manually enter the use case at a minimum and other characteristics required for a Microsoft SQL Server configuration you are planning to deploy and the savings calculator will determine the best file system based on cost. 
