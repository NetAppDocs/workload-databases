...
== About this task
You can explore savings for your database workloads on EBS, FSx for Windows File Server, and on-premises storage using the Dashboard, Inventory tab, or Explore savings tab. In all cases, you can use the savings calculator to compare various cost components of running Microsoft SQL Server workloads like storage, compute, SQL license, snapshots, and clones for your database workloads on FSx for ONTAP file systems against Elastic Block Store (EBS), FSx for Windows File Server, and on-premises storage.

If Workload Factory finds cost savings on FSx for ONTAP, you can deploy Microsoft SQL over FSx for ONTAP directly from the savings calculator in the Workload Factory console. When you have multiple Microsoft SQL Server instances over Elastic Block Store, FSx for Windows File Server, or on-premises storage, we'll recommend an FSx for ONTAP configuration with a single SQL instance.

.Potential savings for all database workloads
Visit the Databases *Dashboard* in the Workload Factory console to get an overview of potential cost savings for running all of your database workloads on FSx for ONTAP. The *Potential savings* tile shows your database workloads on EBS and FSx for Windows File Server, potential cost savings, savings percentage, and a bar graph.
...
The calculator checks your SQL Server deployment to make sure the resources and features used match the SQL Server edition. Here are the key factors and conditions the calculator checks before recommending a downgrade to Standard Edition:
...
If the assessed SQL Server instance doesn't utilize any of the above Enterprise features and meets the resource constraints, the calculator will recommend downgrading the license to Standard Edition. The calculator helps you optimize SQL Server licensing costs without reducing performance or functionality.
...
Customize the fields in the calculator to get the most accurate savings estimate. 

==== Customize savings for Microsoft SQL Server hosts
...
To switch to FSx for ONTAP, follow the instructions to <<Deploy Microsoft SQL Server on AWS EC2 using FSx for ONTAP,deploy Microsoft SQL Server on AWS EC2 using FSx for ONTAP file systems>>.

==== Customize savings for Oracle hosts on EBS
Customize the fields in the calculator to get the most accurate savings estimate for your Oracle hosts running on Amazon Elastic Block Store (EBS). 

.Steps
. Log in using one of the link:https://docs.netapp.com/us-en/workload-setup-admin/console-experiences.html[console experiences^].
. Select the menu image:workloads-menu-icon.png[The hamburger menu icon is used to navigate to workloads like storage, eda, ai, databases, vmware, and administration.] and then select *Databases*.
. From Databases, select *Explore savings* then *Oracle on EBS*. 
. In the Savings calculator, provide the following details: 
.. *Region*: Select a region from the dropdown menu. 
.. *Deployment model*: Select a deployment model from the dropdown menu.
.. *Oracle edition*: Select the Oracle edition from the dropdown menu.
.. *Monthly data change rate (%)*: Enter the percentage that clone and snapshot data changes on average per month.  
.. *Snapshot frequency*: Select a snapshot frequency from the dropdown menu. 
.. *Number of cloned copies*: Enter the number of cloned copies in the EBS configuration.
.. *Monthly Oracle BYOL cost ($)*: Optionally, enter the monthly Oracle BYOL cost in dollars.  
.. Under EC2 specifications, provide the following: 
+
* *Machine description*: Optionally, enter a name to describe the machine.
* *Instance type*: Select the EC2 instance type from the dropdown menu. 
...
Workload Factory enters the detected host characteristics so that you can explore savings automatically.
...
==== Explore savings for detected Microsoft SQL Server hosts
...
. Log in using one of the link:https://docs.netapp.com/us-en/workload-setup-admin/console-experiences.html[console experiences^].
. Select the menu image:workloads-menu-icon.png[The hamburger menu icon is used to navigate to workloads like storage, eda, ai, databases, vmware, and administration.] and then select *Databases*.
. From the Databases menu, select *Explore savings*.
. In Explore savings, select the *SQL Server on Elastic Block Store (EBS)* tab.
+
If Workload Factory detects EBS hosts, it redirects you to the Explore savings tab. If it doesn't detect EBS hosts, it redirects you to the calculator to <<Explore savings via customization,explore savings via customization>>.
...
. In the Savings calculator, you can give the following details on clones and snapshots in your EBS storage for a more accurate cost savings estimate.
...
+
If Workload Factory detects FSx for Windows hosts, it redirects you to the Explore savings tab. If it doesn't detect FSx for Windows hosts, it redirects you to the calculator to <<Explore savings via customization,explore savings via customization>>.
...
.. *Snapshot frequency*: Select a snapshot frequency from the menu.
+
If Workload Factory detects FSx for Windows shadow copies, it sets the default value to *Daily*. If it doesn't detect shadow copies, it sets the default value to *No snapshot frequency*.
.. *Clone refresh frequency*: Select the frequency that clones refresh from the menu.
...
. Select the menu image:workloads-menu-icon.png[The hamburger menu icon is used to navigate to workloads like storage, eda, ai, databases, vmware, and administration.] and then select *Databases*.
. From the Databases menu, select *Explore savings*.
. In Explore savings, select engine type *Microsoft SQL Server*. 
. Next, select the *SQL Server on-premises* tab.
. From the SQL Server on-premises tab, select *Assessment script* > *Assessment script information* and review the information provided. 
. Select *Download assessment script*. 
+
The script is a data collection tool based on PowerShell. It gathers and then uploads SQL Server configuration and performance data to Workload Factory. The migration advisor assesses the data and plans FSx for ONTAP deployment for your SQL Server environment.
. Run the script on the SQL Server host with a user that has sufficient privileges. 
. Select *Upload script results* in the SQL Server on-premises tab in Workload Factory.
. From the SQL Server on-premises tab, select up to five database hosts and then select *Explore savings* to run a cost analysis of the SQL Server on-premises host(s) against FSx for ONTAP. 
...
==== Explore savings for detected Oracle hosts
...
. Select the menu image:workloads-menu-icon.png[The hamburger menu icon is used to navigate to workloads like storage, eda, ai, databases, vmware, and administration.] and then select *Databases*.
. From the Databases menu, select *Explore savings*.
. In Explore savings, select the *Oracle on Elastic Block Store (EBS)* tab.
+
If Workload Factory detects EBS hosts, it redirects you to the Explore savings tab. If it doesn't detect EBS hosts, it redirects you to the calculator to <<Explore savings via customization,explore savings via customization>>.
. From *Explore savings*, select one or more database hosts running on EBS and then select *Explore savings*.
. If required, authenticate the database host with Oracle credentials, or by adding missing Oracle permissions.
+
If the Explore savings page doesn't load data after successful authentication, select the *Inventory* tab to reload the data, and then select the *Explore savings* tab again.
. In the Savings calculator, you can give the following details on clones and snapshots in your EBS storage for a more accurate cost savings estimate.
.. *Snapshot frequency*: Select a snapshot frequency from the menu.
...
.. *Add hosts*: Optionally, select up to five detected EBS hosts to include in the savings calculation.
+
Workload Factory consolidates multiple Oracle hosts into a single FSx for ONTAP configuration recommendation to optimize cost savings unless the selected EBS hosts exceed throughput, capacity, or IOPS limits for a single FSx for ONTAP file system.
--
...
. From the Databases menu, select *Explore savings*.
. In Explore savings, select engine type *Microsoft SQL Server*. 
. Next, select the *Oracle Server on-premises* tab.
. From the Oracle Server on-premises tab, select *Assessment script* > *Assessment script information* and review the information provided. 
. Select *Download assessment script*.
+
The script is a data collection tool based on PowerShell. It gathers Oracle Server configuration and performance data.
. Run the script on the Oracle host with a user that has sufficient privileges.
. Select *Upload script results* and then upload the script output file to the Workload Factory console. The migration advisor assesses the data and plans FSx for ONTAP deployment for your Oracle Server environment.
. From the Oracle Server on-premises tab, select the on-premises Oracle Server and then select *Explore savings* to run a cost analysis against FSx for ONTAP. 
. In the Savings calculator, select the region for the on-premises host. 
. Optionally, provide the following details on clones (shadow copies) and snapshots in your on-premises database environment for a more accurate cost savings estimate.  
.. *Snapshot frequency*: Select a snapshot frequency from the menu.
+
...
. For more accurate results, update Compute information and Storage and performance details.
+
Workload Factory consolidates multiple Oracle on-premises databases into a single FSx for ONTAP configuration recommendation to optimize cost savings unless the selected on-premises databases exceed throughput, capacity, or IOPS limits for a single FSx for ONTAP file system.

.On-premises host removal
After you've explored savings for an Oracle on-premises host, you have the option to remove the on-premises host record from Workload Factory. Select the action menu of the Oracle on-premises host and then select *Delete*.
====
