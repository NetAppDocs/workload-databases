...
[%collapsible]
====
Set snapshot copy reserve to 0% and enable snapshot autodelete so snapshots do not consume space needed for active data.
====
...
Disable scheduled snapshots for FSx for ONTAP volumes used by Microsoft SQL Server.
[%collapsible]
====
...
* ASM data disk group, ASM logs disk group, ASM FRA disk group, and ASM archive disk group LUNs: Distribute LUNs across Automatic Storage Management (ASM) disk group resources.
[%collapsible]
====
