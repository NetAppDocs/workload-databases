...
---
= Implement well-architected database configurations in NetApp Workload Factory
:icons: font
:imagesdir: ./media/
...
== About this task
Workload Factory analyzes database configurations daily. The daily analysis provides well-architected status, insights, and recommendations. 

Many recommendations are available when you grant _view, planning, and analysis_ permissions to your AWS account. If you grant _operations and remediation_ permissions, all recommendations are available, and you can automatically fix configuration issues to meet best practices. 

You can review and fix issues from the Databases inventory or the Well-architected dashboard within the Workload Factory console. Because requirements for database configurations vary, you can dismiss the analysis of specific configurations that don't apply to your database environment. This helps you avoid unnecessary alerts and inaccurate optimization results.
...
* Resiliency: includes local snapshots, backup configuration, cross-region replication (CRR), Microsoft SQL High Availability, and clone cleanup
...
* Resiliency: includes cross-region replication (CRR), application-consistent snapshots, backup configuration, and clone cleanup
...
For each configuration, you can view the current value and the recommended value.
...
If you grant _operations and remediation_ permissions in your AWS account, you can perform an on-demand assessment of your Oracle databases. You don't have to register resources to run this assessment. 
...
. In the *Inventory*, select *Oracle* as the engine type. 
. Select *View and fix*. 
. In the Well-architected status tab, select *Analyze now*. 
. View the recommendation for any configuration. Workload Factory includes best practices and potential problems in the recommendations if configurations are not optimized.


== Before you begin fixing, dismissing, and re-activating configurations
The following requirements are necessary for continuous assessment of your database environment and automatic fixing of configuration issues. These requirements are not required for the one-time assessment.
...
* You must register the resource in Workload Factory and the storage type must be FSx for ONTAP. link:register-instance.html[Learn how to register resources.]
...
NOTE: Fixing a configuration may cause downtimes or service interruptions. Make sure you review the recommendation carefully before you choose to fix a configuration.
...
* link:optimize-overview.html[Learn about the configuration analysis for database resources and well-architected status in Workload Factory]
* link:https://docs.netapp.com/us-en/workload-fsx-ontap/create-link.html[Connect to an FSx for ONTAP file system using a link^]
* link:https://docs.netapp.com/us-en/console-well-architected/learn-custom-rules.html[Learn about custom rules^]