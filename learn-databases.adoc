...
** Create on-demand, isolated databases including storage configurations, and sandbox clones that can be used for testing, integration, diagnostics, and training, without affecting production data. 
...
* *Centralized management*: manage your Microsoft SQL Server, Oracle, and PostgreSQL resources on FSx for ONTAP from a single location. From Workload Factory, you can view host-level, instance-level, and database-level attached infrastructure, AWS account and credentials, associated FSx for ONTAP file systems, storage usage including volumes for your NFS-protocol deployments and LUNs for your iSCSI-protocol associations, and check protection status, performance, well-architected status, and error analysis. 
