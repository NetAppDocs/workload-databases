...
* *Centralized management*: manage your Microsoft SQL Server, Oracle, and PostgreSQL resources on FSx for ONTAP from a single location. Workload Factory provides a unified interface that includes host-level, instance-level, and database-level configurations, and storage usage, storage structure, and volume (NFS protocol) and LUN (iSCSI protocol) associations for inventory, performance, and protection management. 
