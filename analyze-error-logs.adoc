...
+
Amazon Bedrock is required so that the AI agent running on the SQL Server or Oracle node from Workload Factory can seamlessly connect with Bedrock and fetch AI-based insights for the identified error logs. 

** *Networking* 
+
The Amazon Bedrock VPC endpoint ensures private communication of your SQL Server or Oracle node with Amazon Bedrock APIs and eliminates public internet exposure. 
Ensure Amazon Bedrock VPC endpoint is associated with the SQL Server or Oracle node's subnet (example: vpce-050cb2f33a1380ffd). 

** *AWS IAM permissions* 
+
The following permissions are required for the EC2 instance profile role associated with the SQL Server or Oracle node and for the AWS credentials associated with Workload Factory.

*** EC2 instance profile role with "bedrock:InvokeModel" permission
+ 
This permission enables the EC2 instance on the corresponding SQL Server or Oracle node to invoke Bedrock models for proactive error investigation and remediation guidance. This profile also ensures secure AI access for tailored insights. 
...
+ 
These permissions verify model availability and configuration in the region of the SQL Server or Oracle node, and ensure reliable, region-specific performance.
