...
The well-architected analysis includes four new sub assessments for the storage configuration analysis for Oracle: dNFS enablement, dNFS consistent IP resolution, dNFS configuration file, and dNFS no shared cache. These assessments relate to enabling and setting up Direct NFS (dNFS) for your Oracle environment, which bypasses the host NFS client and performs NFS file operations directly on an NFS server, improving I/O performance and decreasing the load on the host and the storage system because I/O is performed more efficiently.
