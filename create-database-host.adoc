...
---
= Create a database server in Workload Factory Databases
:icons: font
:imagesdir: ./media/

[.lead]
Create a new Microsoft SQL server, or database host, within Workload Factory Databases. You'll need AWS account credentials and _automate_ permissions. 

== About this task
A new Microsoft SQL server in Workload Factory Databases requires an FSx for ONTAP file system deployment and resources for Active Directory. 

Before you begin, learn about the storage deployment for the database host configuration, Active Directory deployment, Workload Factory modes of operation, and the <<requirements>> to complete this operation.
...
+
2. In case of AWS-managed Active Directory, the security group attached to the directory service gets automatically added to the SQL nodes to allow communication with the Active Directory.  
+
...
+
For a user-managed Active Directory, you can select additional security groups that allows traffic from your existing host farm/Windows clients. The additional security groups should allow communication to this domain from the subnets where EC2 instances for SQL are configured. 

. Under *Application settings*, provide the following: 
.. *Operating system*: Select *Windows server 2016* or *Windows server 2019*. 
...
...	Use custom AMI: This option isn't currently supported.
...
+
FSx for ONTAP custom encryption keys aren't listed based on service applicability. You must choose an appropriate key. 
+
