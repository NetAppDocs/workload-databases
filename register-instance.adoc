...
== About the task
Registering an instance (SQL Server) or database (Oracle) involves three steps: instance or database authentication, FSx for ONTAP authentication, and preparation. To prepare, install AWS, NetApp, and PowerShell modules on the instance or database, and meet the minimum requirements for Workload Factory for Databases features like link:analyze-error-logs.html[error log analysis] or link:optimize-configurations.html[well-architected assessment].
...
AWS GovCloud (US) users must use AWS Secrets Manager to authenticate FSx for ONTAP credentials. For all other users, you can authenticate FSx for ONTAP with either Secrets Manager or file system credentials.
...
+
For GovCloud accounts, the AWS SSM Parameter ARN value is provided in the Parameter Store under Parameter details in the AWS Management Console in the following format: `{fsx:{"username":"fsxadmin","password":"pass"}}`. The AWS SSM Parameter type must be "Secure String" with the name in the format `/netapp/wlmdb/<fsx_filesystemID>`. Learn more about retrieving SSM parameters from link:https://docs.aws.amazon.com/govcloud-us/latest/UserGuide/govcloud-ssm.html[AWS GovCloud documentation^].  
+
If the FSx for ONTAP file system is authenticated, select *Next*.
. To Prepare (step 3), make sure the instance(s) meets the minimum requirements. 
+
...
.. Enter either the AWS SSM Parameter ARN with FSx for ONTAP credentials for GovCloud accounts or the FSx for ONTAP file system user name and password for all other accounts, and then select *Next*. 
+
The AWS SSM Parameter type must be "Secure String" with the name in the format `/netapp/wlmdb/<fsx_filesystemID>`. Enter the value for the AWS SSM Parameter ARN in the following format:
`{fsx:{"username":"fsxadmin","password":"pass"}}`. Learn more about retrieving SSM parameters from link:https://docs.aws.amazon.com/govcloud-us/latest/UserGuide/govcloud-ssm.html[AWS GovCloud documentation^]. 
+
If the FSx for ONTAP file system is authenticated, select *Next*.
