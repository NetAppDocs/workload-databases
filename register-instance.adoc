...
== About the task
Registering an instance (SQL Server) or database (Oracle) has three steps - instance or database authentication, FSx for ONTAP authentication, and preparation. Preparation involves making sure that all AWS, NetApp, and PowerShell modules are installed on the instance and that the minimum requirements for a Workload Factory features like log error analysis or well-architected review are met.
