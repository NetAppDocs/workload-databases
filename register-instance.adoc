...
. To Authenticate instances (step 1), select to *Use the same credentials for all instances* or *Manage credentials manually*. If instances are already authenticated, select *Next*.
. Provide the following details based on the authentication mode selected:
...
+
All prerequisites for a single capability like *Review well-architected issues and recommendations* must be complete to register the database. 
...
+
Let's say that you want Workload Factory to review the well-architected issues for your databases, provide recommendations, and automatically fix the issues. In this case, you must complete all prerequisites listed under *Review well-architected issues and recommendations* and *Fix well-architected issues* capabilities.
...
. To Authenticate databases (step 1), select to *Use the same credentials for all instances* or *Manage credentials manually*. Then provide the following Oracle credentials and select *Next*:

* *Oracle user authentication*: Enter the Oracle user name and password.
* *ASM grid user authentication*: Optional. If the Oracle database uses Automatic Storage Management (ASM), enter the Oracle ASM (grid) user name and password.
+
...
* Select to *Use the same credentials for all resources* or *Manage credentials manually*. 
* Enter the FSx for ONTAP file system user name and password, and then select *Next*. If the FSx for ONTAP file system is already authenticated, select *Next*.
. To Prepare (step 3), make sure the database meets required prerequisites.
