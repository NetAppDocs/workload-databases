...
== About the task
Registering an instance (SQL Server) or database (Oracle) includes three steps - instance or database authentication, FSx for ONTAP authentication, and preparation. Preparation involves making sure that all AWS, NetApp, and PowerShell modules are installed on the instance and that the minimum requirements for a Workload Factory features like log error analysis or well-architected review are met.
...
. To Authenticate instances (step 1), do the following: 
...
+
If instances are already authenticated, select *Next*.
