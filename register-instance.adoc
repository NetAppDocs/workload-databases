...
.. Enter either the AWS GovCloud Secrets Manager endpoint for GovCloud accounts or the FSx for ONTAP file system user name and password for all other users, and then select *Next*. 
+
...
. To Authenticate FSx for ONTAP (step 2), do the following and then select *Next*:
* Select to *Use the same credentials for all resources* or *Manage credentials manually*. 
* Enter either the AWS GovCloud Secrets Manager endpoint for GovCloud accounts or the FSx for ONTAP file system user name and password for all other users.
+
If the FSx for ONTAP file system is authenticated, select *Next*.
