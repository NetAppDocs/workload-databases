...
NOTE: When using Terraform from Codebox, the code you copy or download hides `fsxadmin` and `vsadmin` passwords. You'll need to re-enter the passwords when you run the code. Additionally, you'll need to include the following permissions: `iam:TagRole` and `iam:TagInstanceProfile`. 
