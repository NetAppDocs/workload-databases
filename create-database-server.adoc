...
Microsoft SQL Server deployment models both require LUN creation using the iSCSI storage protocol. Workload Factory requires Microsoft Multi-path I/O (MPIO) installation when configuring LUNs for SQL Server over FSx for ONTAP. MPIO is installed and maintained based on AWS and NetApp best practices. 
