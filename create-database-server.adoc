...
summary: "Create a new Microsoft SQL Server within NetApp Workload Factory for Databases." 
---
= Create a Microsoft SQL Server in NetApp Workload Factory for Databases
:icons: font
:imagesdir: ./media/
...
You must link:https://docs.netapp.com/us-en/workload-setup-admin/add-credentials.html[grant _database host creation_ permissions^] in your AWS account to create a new database host in Workload Factory. 
...
. In the Databases tile, select *Deploy host* and then select *Microsoft SQL Server* from the menu.
. Select *Quick create*.  
...
+
For GovCloud accounts, enter the value for the AWS SSM Parameter ARN in the following format:
`{fsx:{"username":"fsxadmin","password":"pass"}}`.
+
The value is provided in the Parameter Store under Parameter details in the AWS Management Console. The AWS SSM Parameter type must be "Secure String" with the name in the format `/netapp/wlmdb/<anyname>`. Learn more about retrieving SSM parameters from link:https://docs.aws.amazon.com/govcloud-us/latest/UserGuide/govcloud-ssm.html[AWS GovCloud documentation^].  
+
...
+
For GovCloud accounts, enter the AWS SSM Parameter ARN value for Active Directory credentials in the following format: `{fsx:{"username":"fsxadmin","password":"pass"}}`. The AWS SSM Parameter type must be "Secure String" with the name in the format `/netapp/wlmdb/<anyname>`. Learn more about retrieving SSM parameters from link:https://docs.aws.amazon.com/govcloud-us/latest/UserGuide/govcloud-ssm.html[AWS GovCloud documentation^].
...
+
For GovCloud accounts, enter the AWS SSM Parameter ARN value for FSx for ONTAP credentials in the following format: `{fsx:{"username":"fsxadmin","password":"pass"}}`. The AWS SSM Parameter type must be "Secure String" with the name in the format `/netapp/wlmdb/<anyname>`. Learn more about retrieving SSM parameters from link:https://docs.aws.amazon.com/govcloud-us/latest/UserGuide/govcloud-ssm.html[AWS GovCloud documentation^].
+
A new FSx for ONTAP file system may add 30 minutes or more of installation time. 
... *Select an existing FSx for ONTAP*: Select the FSx for ONTAP name, and enter a user name and password for the file system. 
+
...
.. *Availability zones*: Select availability zones and subnets according to the deployment model you selected. Subnets should not share the same route table for high availability. 
+
NOTE: FCI deployments are only supported on Multiple Availability Zone (MAZ) FSx for ONTAP configurations. 
+
...
.. *Security group*: A workload security group is created to allow ports and protocols required for Microsoft SQL and Windows cluster communication on nodes. In addition, you can select one or more security groups.
...
+
For AWS-managed Active Directories, the DNS IP address(es) appear in the dropdown menu.
... Enter the user name and password for the Active Directory domain. 
+
For GovCloud accounts, enter the AWS SSM Parameter ARN value for Active Directory credentials in the following format: `{fsx:{"username":"fsxadmin","password":"pass"}}`. The AWS SSM Parameter type must be "Secure String" with the name in the format `/netapp/wlmdb/<anyname>`. Learn more about retrieving SSM parameters from link:https://docs.aws.amazon.com/govcloud-us/latest/UserGuide/govcloud-ssm.html[AWS GovCloud documentation^].
... *Preferred domain controller*: Optionally, enter the preferred domain controller to use for the Active Directory to join. 
... *Preferred organizational unit path*: Optionally, enter the preferred organizational unit (OU) in the Active Directory to join.
... *Target Active Directory group*: Optionally, enter the target Active Directory group to add the computers to.
. Under *Infrastructure settings*, provide the following:  
.. *DB Instance type*: Select the database instance type from the dropdown menu. 
.. *FSx for ONTAP system*: Create a new FSx for ONTAP file system or use an existing FSx for ONTAP file system. 
... *Create new FSx for ONTAP*: Enter user name and password.
+
For GovCloud accounts, enter the AWS SSM Parameter ARN value for the FSx for ONTAP credentials in the following format: `{fsx:{"username":"fsxadmin","password":"pass"}}`. The AWS SSM Parameter type must be "Secure String" with the name in the format `/netapp/wlmdb/<anyname>`. Learn more about retrieving SSM parameters from link:https://docs.aws.amazon.com/govcloud-us/latest/UserGuide/govcloud-ssm.html[AWS GovCloud documentation^].
+
A new FSx for ONTAP file system may add 30 minutes or more of installation time. 
...
.What's next
Now you can link:create-database.html[create a database in Workload Factory for Databases]. 
