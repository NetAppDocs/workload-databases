...
---
= Create a PostgreSQL server in NetApp Workload Factory for Databases
:icons: font
:imagesdir: ./media/
...
== Before you begin
You must have link:https://docs.netapp.com/us-en/workload-setup-admin/add-credentials.html[grant _database host creation_ permissions^] in your AWS account to create a new database host in workload factory. 
...
.. *Security group*: A workload security group is created to allow ports and protocols required for PostgreSQL. In addition, you can select one or more security groups.
...
+
For GovCloud accounts, enter the AWS SSM Parameter ARN value for the credentials in the following format: `{fsx:{"username":"fsxadmin","password":"pass"}}`. The AWS SSM Parameter type must be "Secure String" with the name in the format `/netapp/wlmdb/<anyname>`. Learn more about retrieving SSM parameters from link:https://docs.aws.amazon.com/govcloud-us/latest/UserGuide/govcloud-ssm.html[AWS GovCloud documentation^].
. Under *Connectivity*, select a key pair to connect securely to your instance.
. Under *Infrastructure settings*, provide the following:  
...
.. *Simple Notification Service*: Optionally, you can enable the Simple Notification Service (SNS) for this configuration by selecting an SNS topic for PostgreSQL from the dropdown menu. 
